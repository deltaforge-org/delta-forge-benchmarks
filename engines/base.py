...
class StepResult:
    ...
    """Optional per-step structured metrics that don't fit the fixed
    columns above. Used by the graph GPU-vs-CPU workload to carry the
    Cypher per-phase breakdown parsed from `SHOW STATS ACTUAL` rows
    (e.g. {"gpu.used_gpu": "true", "gpu.gpu_wall_ms": "12.3",
    "algorithm.name": "pagerank", "expand.expand_ms": "4.1"}). None for
    steps that produce no extra metrics."""
