...
#   5. Start delta-forge-compute. Wait for compute /health on :3031.
...
# ----- 4. Start delta-forge-compute (compute node) ----------------------------
#
# The released compute node hardcodes ComputeMode::Remote and reads its env from
# delta-forge-compute/src/bin/compute.rs. Required:
...
echo "[entrypoint] starting delta-forge-compute (control=${CONTROL_PLANE_URL}, bind=${BIND_HOST}:${BIND_PORT})"
nohup delta-forge-compute > "${LOG_DIR}/worker.log" 2>&1 &
WORKER_PID=$!
echo $WORKER_PID > /var/run/delta-forge-compute.pid
