-- DeltaForge CTAS perf test: 5M-row x 43-col dim_customer shape, written
-- via df_generate_table(...) AS SELECT into a fresh Delta table.
--
-- Pair with df_insert.sql (same projection, INSERT path) and duckdb_copy.sql
-- (same shape, plain parquet) for a full head-to-head. Current numbers and
-- known issues live in BASELINE.md.

CREATE SCHEMA IF NOT EXISTS pbi.bench;
DROP DELTA TABLE IF EXISTS pbi.bench.dim_customer_ctas WITH FILES;

CREATE DELTA TABLE pbi.bench.dim_customer_ctas
LOCATION 'B:/odbc_df/df-demo/perf_test/dim_customer_ctas' AS
SELECT
    g.customer_id,
    g.customer_code,
    g.salutation,
...
SELECT COUNT(*) AS rows FROM pbi.bench.dim_customer_ctas;
