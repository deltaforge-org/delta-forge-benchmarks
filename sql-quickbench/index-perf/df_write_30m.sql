-- index-perf: write a 30M-row fixture. Schema is identical to
-- df_write_10m.sql; only the row count and LOCATION differ.
-- Running both sizes back-to-back with the same index queries
-- shows how no-index scan time grows linearly with rows while
-- the indexed path stays roughly constant.

CREATE SCHEMA IF NOT EXISTS pbi.bench;
DROP DELTA TABLE IF EXISTS pbi.bench.idx_perf_customer_30m WITH FILES;

CREATE DELTA TABLE pbi.bench.idx_perf_customer_30m (
...
INSERT INTO pbi.bench.idx_perf_customer_30m
SELECT
    g.customer_id,
    g.customer_code,
    g.salutation,
    ...
    g.churn_risk_score,
    g.segment
FROM df_generate_table(30000000, '[
    ...
    {"type": "cyclic_lookup", "name": "street_part",   "values": ["Main St","Oak Ave","Maple Dr","Cedar Ln","Elm St","Pine Rd","Birch Way","Walnut Ct","Spruce Pl","Willow Ln"]},
...
SELECT COUNT(*) AS rows_written FROM pbi.bench.idx_perf_customer_30m;
