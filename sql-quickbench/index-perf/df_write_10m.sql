-- index-perf: write a 10M-row fixture for the bench. Self-contained,
-- writes to B:/tmp/index-perf-data so the bench has no external
-- dependency on the read-perf-10m fixture.
--
-- Schema mirrors a realistic customer-dimension shape (43 columns,
-- mix of high- and low-cardinality, mix of types) so the index
-- comparison reflects production query patterns rather than a
-- toy table.

CREATE SCHEMA IF NOT EXISTS pbi.bench;
DROP DELTA TABLE IF EXISTS pbi.bench.idx_perf_customer_10m WITH FILES;

CREATE DELTA TABLE pbi.bench.idx_perf_customer_10m (
...
LOCATION 'B:/tmp/index-perf-data/idx_perf_customer_10m';

INSERT INTO pbi.bench.idx_perf_customer_10m
SELECT
    g.customer_id,
    g.customer_code,
    g.salutation,
...
SELECT COUNT(*) AS rows FROM pbi.bench.idx_perf_customer_10m;
