-- DuckDB writes the 5M-row dim_customer shape into a *directory* of parquet
-- files so DeltaForge can wrap it in-place via CONVERT TO DELTA. Same 43
-- columns as ../write-perf/duckdb_copy.sql; only the destination differs.
--
-- The point of this ad-hoc benchmark is to feed DF's read path the exact
-- bytes (and row-group layout) that DuckDB wrote, so any read-time delta
...
TO 'B:/odbc_df/df-demo/perf_test/dim_customer_duck/data.parquet'
(FORMAT PARQUET, COMPRESSION 'snappy');

SELECT COUNT(*) AS rows FROM read_parquet('B:/odbc_df/df-demo/perf_test/dim_customer_duck/*.parquet');
