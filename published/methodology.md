...
## Graph GPU-vs-CPU contract

The [Graph GPU vs CPU](graph_gpu.md) benchmark is a different shape from
the read/write benches: one engine (DeltaForge), two **devices** (CPU vs
GPU), several graph sizes. Its contract:

- **Fair A/B by construction.** GPU execution in DeltaForge is opt-in via
  the `ON GPU` Cypher hint. The CPU arm is `USE <graph> CALL algo.X(...)`;
  the GPU arm is the identical statement with `ON GPU THRESHOLD 1`
  inserted. Nothing else differs. `THRESHOLD 1` forces GPU dispatch at
  every size so a small graph never falls back to CPU silently.

- **Only confirmed-GPU algorithms.** PageRank, Connected Components,
  Louvain, Betweenness, Triangle Count. Each has a GPU kernel verified by
  the `graph-gpu-10m-finance` correctness demo, so the GPU column cannot
  silently run CPU code.

- **Headline = warm-median synchronous `execution_time_ms`.** Unlike the
  SQL streaming path, a Cypher `CALL` runs synchronously inside the
  handler, so its `execution_time_ms` already covers plan + compute +
  result drain end to end. There is nothing to wrap in `SHOW STATS`
  (which is SQL-only and rejects Cypher), so the Cypher number is read
  directly.

- **One-time topology build is untimed setup.** `CREATE GRAPHCSR`
  (CSR sidecar) and `CREATE GRAPHGPU` (GPU buffer sidecar) run once in
  setup, exactly as the read benches pay table-attach cost in untimed
  setup. The warm-median therefore measures repeated-query compute on the
  cached topology, the number that matters for interactive analytics.

- **Per-phase GPU breakdown is not exposed.** The host->device transfer /
  kernel / readback split lives only in the engine's debug logs in this
  build; it is not reachable from a SQL/CLI client, so it is not
  published. The end-to-end time is what a client experiences.

- **Synthetic, reproducible graphs.** 1M / 5M / 10M / 30M nodes at
  ~4.5x edges per node, generated in-engine (`df_generate_table` +
  `generate_series`), no on-disk input. The 30M tier needs ~32 GB RAM;
  set `GRAPH_BENCH_SIZES_M="1,5,10"` on smaller hosts.

This is an honest crossover study: the table includes the algorithms
where the multi-core CPU stays competitive (sparse, memory-bandwidth
bound) right beside the ones where the GPU pulls ahead (compute bound),
so the reader can see the line rather than be told where it is.
...
| [`workloads/graph_gpu_vs_cpu.py`](../workloads/graph_gpu_vs_cpu.py) | Graph GPU-vs-CPU workload (synthetic graphs, CPU/GPU algorithm matrix) |
| [`reports/build_graph_gpu_report.py`](../reports/build_graph_gpu_report.py) | Graph GPU-vs-CPU crossover report builder |
| [`engines/df_engine.py`](../engines/df_engine.py) | DeltaForge adapter (`SHOW STATS ACTUAL` wrap for SQL, plain run for Cypher, OPEN preamble) |
